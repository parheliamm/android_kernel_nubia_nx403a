...
#include "wcd9310.h"
#if 1
//ztemt audio add by sunli,  use to compat with American-European standard headset detection.03-25-2013
#define Compatible_headset_detection_gpio  57
#endif
...
#if 1
		//ztemt audio add by sunli,  use to compat with American-European standard headset detection.03-25-2013
		gpio_set_value_cansleep(Compatible_headset_detection_gpio, 0);	  //sunli
		//pr_err("[ztemt]Compatible_headset_detection_gpio  pull down");
#endif
	} else {
...
static void tabla_hs_gpio_handler(struct snd_soc_codec *codec)
{
    ...
    if(tabla->current_plug == PLUG_TYPE_NONE)
        {
	    usleep_range(TABLA_GPIO_IRQ_DEBOUNCE_TIME_US*100,
			TABLA_GPIO_IRQ_DEBOUNCE_TIME_US*100);
        pr_debug("GJL the delay is inserted\n");
        }
#else
	...
	if ((tabla->current_plug == PLUG_TYPE_NONE) && insert) {
		...
		//ztemt audio add by sunli,  use to compat with American-European standard headset detection.03-25-2013
		if (is_first_time==1) {
			rc = gpio_request(Compatible_headset_detection_gpio, "headset");
			if (rc) {
				pr_err("request Compatible_headset_detection_gpio failed rc=%d\n", rc);
			} else {
				pr_err("%s: enter, Compatible_headset_detection_gpio pull up \n", __func__);}
			is_first_time=0;
		}
...
static const struct tabla_reg_mask_val tabla_1_1_reg_defaults[] = {
...
#if defined (CONFIG_ZTEMT_AUDIO_Z5mini) || defined (CONFIG_ZTEMT_AUDIO_Z5)
	/*ztemt audio add by zmwu 20121016, set bit2 0  for handset second amic4 are single end mode*/  
	...
	/*ztemt audio add by zmwu 20121010, set bit2 0 and bit 5 0 for both headset amic2 and handset amic3 are single end mode*/
...
#if defined (CONFIG_ZTEMT_AUDIO_Z5mini) || defined (CONFIG_ZTEMT_AUDIO_Z5)
	/*ztemt audio add by zmwu 20121016, set bit2 0  for handset second amic4 are single end mode*/  
	{TABLA_A_MICB_1_INT_RBIAS, 0x24, 0x00},
#else
	{TABLA_A_MICB_1_INT_RBIAS, 0x24, 0x24},
#endif
#if defined (CONFIG_ZTEMT_AUDIO_Z5smini) || defined (CONFIG_ZTEMT_AUDIO_Z5mini) || defined (CONFIG_ZTEMT_AUDIO_Z5)
	/*ztemt audio add by zmwu, set bit2 0 and bit 5 0 for both headset amic2 and handset amic3 are single end mode*/
	{TABLA_A_MICB_2_INT_RBIAS, 0x24, 0x00},
#else
