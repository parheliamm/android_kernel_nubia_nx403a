...
#define MIPI_TOSHIBA_1080P_PANEL_NAME "mipi_zte_sharp"
#if defined( CONFIG_ZTEMT_LCD_Z5MINI)	
#define MIPI_SHARO_720P_NT35590_PANEL_NAME "mipi_zte_sharp_nt35590"
#define MIPI_SHARO_720P_R63415_PANEL_NAME "mipi_zte_sharp_r63415"
#endif
#if defined( CONFIG_ZTEMT_LCD_Z5MINI2)	
#define MIPI_SHARO_720P_R69431_PANEL_NAME "mipi_zte_sharp_r69431"
#endif
...
#if defined( CONFIG_ZTEMT_LCD_V5)
	if (!strncmp(name, MIPI_TOSHIBA_1080P_PANEL_NAME,
			strnlen(MIPI_TOSHIBA_1080P_PANEL_NAME,
				PANEL_NAME_MAX_LEN)))
			return 0;
#elif defined( CONFIG_ZTEMT_LCD_Z5MINI2_SIZE_4P7)	
	if (!strncmp(name, MIPI_SHARO_720P_R69431_PANEL_NAME,
			strnlen(MIPI_SHARO_720P_R69431_PANEL_NAME,
				PANEL_NAME_MAX_LEN)))
			return 0;
#elif defined( CONFIG_ZTEMT_LCD_Z5MINI_SIZE_4P7)	
	if (!strncmp(name, MIPI_SHARO_720P_NT35590_PANEL_NAME,
			strnlen(MIPI_SHARO_720P_NT35590_PANEL_NAME,
				PANEL_NAME_MAX_LEN)))
			return 0;	
#elif defined( CONFIG_ZTEMT_LCD_Z5MINI_SIZE_4P5)		
	if (!strncmp(name, MIPI_SHARO_720P_R63415_PANEL_NAME,
			strnlen(MIPI_SHARO_720P_R63415_PANEL_NAME,
				PANEL_NAME_MAX_LEN)))
			return 0;	
#else
	u32 version;
...
#endif
#ifdef CONFIG_ZTEMT_LCD_8064_COMMON
	.mdp_bw_ab_factor = 205,
	.mdp_bw_ib_factor = 215,
#else
...
#define LCD_GPIO_RESET PM8921_GPIO_PM_TO_SYS(25)
#define LCD_GPIO_BKLIGHT PM8921_GPIO_PM_TO_SYS(26)

#if defined( CONFIG_ZTEMT_LCD_Z5MINI2)
#define LCD_GPIO_APWR_ENP 86
#define LCD_GPIO_APWR_ENN 81
#else
#define LCD_GPIO_APWR_EN 86
#endif
#define LCD_MPP2 PM8921_MPP_PM_TO_SYS(2)
#define LCD_MPP3 PM8921_MPP_PM_TO_SYS(3)

#ifdef CONFIG_ZTEMT_LCD_Z5MINI2
/*lcd shutdown when restart and poweroff,mayu add ,2013.12.2*/
static void zte_panel_power_shutdown(void);
#endif

static int zte_sharp_gpio[] = {LCD_GPIO_RESET, LCD_MPP2};
static struct mipi_dsi_panel_platform_data zte_sharp_pdata = {
	.gpio = zte_sharp_gpio,
#ifdef CONFIG_ZTEMT_LCD_Z5MINI2
  .shutdown = zte_panel_power_shutdown,
#endif
};
...
static struct platform_device mipi_dsi_sharp_panel_device = {
	...
	.id = 0,
	.dev = {
			.platform_data = &zte_sharp_pdata,
	}
};

static int mipi_dsi_zte_panel_power(int on)
{
	static struct regulator *reg_l11, *reg_l2,*reg_lvs7;
#if defined( CONFIG_ZTEMT_LCD_Z5MINI)		
	...
	if (is_firsttime) {
		...
		reg_l11 = regulator_get(NULL,
						"sharp_panel_1p8");
		if (IS_ERR(reg_l11)) {
				pr_err("could not get sharp_panel_1p8, rc = %ld\n",
						PTR_ERR(reg_l11));
				return -ENODEV;
		}
...
#if defined( CONFIG_ZTEMT_LCD_Z5MINI)		
		reg_l22 = regulator_get(NULL,
						"sharp_panel_2p8");
		if (IS_ERR(reg_l22)) {
				pr_err("could not get sharp_panel_2p8, rc = %ld\n",
						PTR_ERR(reg_l22));
				return -ENODEV;
		}
...
#endif		
		rc = gpio_request(LCD_GPIO_RESET, "lcd_reset");
		if (rc) {
			pr_err("request LCD_GPIO_RESET failed rc=%d\n", rc);
		}
		/*added by congshan 20121026 start*/
	#ifdef CONFIG_ZTEMT_8064_CONS_DISPLAY
	#else
		#ifndef CONFIG_ZTEMT_LCD_Z5MINI2
		gpio_set_value_cansleep(LCD_GPIO_RESET, 0);
		mdelay(3);
		#endif
	#endif
		rc = gpio_request(LCD_GPIO_BKLIGHT, "lcd_backlight");
		if (rc) {
			pr_err("request LCD_GPIO_BKLIGHT failed rc=%d\n", rc);
		}

	#if defined( CONFIG_ZTEMT_LCD_Z5MINI2)
		rc = gpio_request(LCD_GPIO_APWR_ENP, "apwr_enp");
		if (rc) {
			pr_err("request LCD_GPIO_APWR_ENP failed rc=%d\n", rc);
		}
		rc = gpio_request(LCD_GPIO_APWR_ENN, "apwr_enn");
		if (rc) {
			pr_err("request LCD_GPIO_APWR_ENN failed rc=%d\n", rc);
		}
	#else
		rc = gpio_request(LCD_GPIO_APWR_EN, "apwr_en");
		if (rc) {
			pr_err("request LCD_GPIO_APWR_EN failed rc=%d\n", rc);
		}
	#endif
		rc = gpio_request(LCD_MPP2, "lcd_id0");
		if (rc) {
			pr_err("request LCD_MPP2 failed rc=%d\n", rc);
		}
		rc = gpio_request(LCD_MPP3, "lcd_id1");
		if (rc) {
			pr_err("request LCD_MPP3 failed rc=%d\n", rc);
		}
		gpio_get_value_cansleep(LCD_MPP2);
		gpio_get_value_cansleep(LCD_MPP3);
		is_firsttime = 0;
	}

	if (on) {
#ifndef CONFIG_ZTEMT_LCD_Z5MINI2
		gpio_set_value_cansleep(LCD_GPIO_BKLIGHT, 1);
#endif
#if defined( CONFIG_ZTEMT_LCD_Z5MINI2)
		pr_debug("yls==========%s:line%d===========.\n",__func__,__LINE__);
		...
		if (rc) {
			pr_err("enable reg_l11 failed, rc=%d\n", rc);
			return -ENODEV;
		}
		mdelay(5);

		if (SHARP_R69431 == mini2_get_lcd_type()) {
			gpio_direction_output(LCD_GPIO_APWR_ENP, 1);
			mdelay(2);
			gpio_direction_output(LCD_GPIO_APWR_ENN, 1);
			mdelay(6);
			...
			if (is_firston) {
				gpio_set_value_cansleep(LCD_GPIO_RESET, 0);
				msleep(5);
				gpio_direction_output(LCD_GPIO_APWR_ENN, 0);
				mdelay(5);
				gpio_direction_output(LCD_GPIO_APWR_ENP, 0);
				mdelay(10);
					

				gpio_set_value_cansleep(LCD_GPIO_APWR_ENP, 1);		
				mdelay(5);		
				gpio_set_value_cansleep(LCD_GPIO_RESET, 1);		
				mdelay(20);		
				gpio_set_value_cansleep(LCD_GPIO_APWR_ENN, 1);		
				mdelay(15);		
				gpio_set_value_cansleep(LCD_GPIO_RESET, 0);		
				mdelay(5);		
				gpio_set_value_cansleep(LCD_GPIO_RESET, 1);		
		...
		if (rc) {
			pr_err("enable reg_l22 failed, rc=%d\n", rc);
			return -ENODEV;
		}	
		msleep(20);
#endif		
		...
		gpio_direction_output(LCD_GPIO_APWR_EN, 1);
		msleep(3);
		rc = regulator_set_optimum_mode(reg_l2, 100000);
		if (rc < 0) {
			pr_err("set_optimum_mode l2 failed, rc=%d\n", rc);
		...
		gpio_set_value_cansleep(LCD_GPIO_RESET, 1);
		//pr_err("sss LCD_GPIO_RESET=%d", gpio_get_value(LCD_GPIO_RESET));
		//pr_err("sss LCD_MPP2=%d LCD_MPP3=%d\n", gpio_get_value_cansleep(LCD_MPP2), gpio_get_value_cansleep(LCD_MPP3));
		msleep(10);
#endif
	} else {
#ifndef CONFIG_ZTEMT_LCD_Z5MINI2
		gpio_set_value_cansleep(LCD_GPIO_BKLIGHT, 0);
#endif
#if defined( CONFIG_ZTEMT_LCD_Z5MINI2)
		if (SHARP_R69431 == mini2_get_lcd_type()) {
			pr_debug("yls==========sharp off %s:line%d===========.\n",__func__,__LINE__);
			gpio_set_value_cansleep(LCD_GPIO_APWR_ENN, 0);
			mdelay(10);
			gpio_set_value_cansleep(LCD_GPIO_APWR_ENP, 0);
			mdelay(10);
			gpio_set_value_cansleep(LCD_GPIO_RESET, 0);
			mdelay(5);
		} else {
			pr_debug("yls==========tianma off %s:line%d===========.\n",__func__,__LINE__);
			gpio_set_value_cansleep(LCD_GPIO_RESET, 0);		
			msleep(5);		
			gpio_set_value_cansleep(LCD_GPIO_APWR_ENN, 0);		
			msleep(5);		
		...
		gpio_set_value_cansleep(LCD_GPIO_RESET, 0);
		msleep(20);
		gpio_set_value_cansleep(LCD_GPIO_APWR_EN, 0);
		msleep(120);
#if defined( CONFIG_ZTEMT_LCD_Z5MINI)		
