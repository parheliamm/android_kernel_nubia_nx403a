...
#ifdef CONFIG_ZTEMT_SENSORS_COMPASS_AKM8963
...
#ifdef CONFIG_NFC_PN544
#define PN544_NFC_SW_UPDATE (13)
#define PN544_NFC_nVEN      PM8921_GPIO_PM_TO_SYS(7)
#define PN544_NFC_GPIO_INT  (55)
static int pn544_nfc_request_resources(struct i2c_client *client)
{
	printk("[%s] pn544_nfc_request_resources start.\n", __func__);
	/*
	rc = pmapp_clock_vote("NNFC", PMAPP_CLOCK_ID_A1, PMAPP_CLOCK_VOTE_ON);
	if (rc < 0) {
		printk(KERN_ERR "%s: unable to vote for a1 clk (%d),UBMC PMAPP_CLOCK_VOTE_ON.\n", __func__, rc);
		return -EIO;
	}
*/
	/* Get and setup the HW resources for the device */
	#if 0
	rc = pmapp_clock_vote("NNFC ", PMAPP_CLOCK_ID_A1, PMAPP_CLOCK_VOTE_PIN_CTRL);
	if (rc < 0) {
		printk(KERN_ERR "%s: unable to vote for a1 clk (%d)\n", __func__, rc);
		return -EIO;
	}
	#endif
...
static int pn544_irq_status(void)
{
...
static struct pn544_nfc_platform_data pn544_nfc_data = {
    ...
    .irq_status = pn544_irq_status,
};
static struct i2c_board_info pn544_i2c_device[] = {
	{
		I2C_BOARD_INFO("pn544", 0x28), 
		.platform_data = &pn544_nfc_data,
		.irq = MSM_GPIO_TO_INT(PN544_NFC_GPIO_INT),
	},
};
 ...
 */
#if defined (CONFIG_ZTEMT_AUDIO_Z5smini) || defined (CONFIG_ZTEMT_AUDIO_Z5mini) || defined (CONFIG_ZTEMT_AUDIO_Z5)
/*ztemt audio add by zmwu, use for config micbias volt, and both micbias1(as handset mic)&2(as headset mic) is set 1.8V,
only one cilt2 is connected while others is dni state. */
...
#ifdef CONFIG_TOUCHSCREEN_ATMEL_MXT //yfliu remove the useless code
...
#endif
#ifdef CONFIG_TOUCHSCREEN_CYTTSP_I2C //yfliu remove the useless code
...
//===================================== sensor platform data begain==========================================//

//virtual sensor dev
#ifdef CONFIG_ZTEMT_VIRTUAL_SENSOR_DEV
static struct zte_sensors_power_regulator sensors_power_regulator_data[] = {
    {
		.name = "8921_l9_sensor",
		.min_uV = ZTE_SENSORS_VTG_MIN_UV,
		.max_uV = ZTE_SENSORS_VTG_MAX_UV,
		.hpm_load_uA = ZTE_SENSORS_CURR_24HZ_UA,
		.lpm_load_uA = ZTE_SENSORS_SLEEP_CURR_UA,
	},
	{
		.name = "8921_vs4_sensor",
		.min_uV = ZTE_SENSORS_I2C_VTG_MIN_UV,
		.max_uV = ZTE_SENSORS_I2C_VTG_MAX_UV,
		.hpm_load_uA = ZTE_SENSORS_I2C_CURR_UA,
		.lpm_load_uA = ZTE_SENSORS_I2C_SLEEP_CURR_UA,
	},
};

struct ztemt_virtual_sensor_dev_platform_data ztemt_virtual_sensor_dev_pdata = {
    .num_regulators = ARRAY_SIZE(sensors_power_regulator_data),
	.regulator_info = sensors_power_regulator_data,
};
#endif
...
struct regulator_bulk_data regs_8921_l9_sensor[] = {
	{.supply = "8921_l9_sensor", .min_uV = 3000000, .max_uV = 3000000},
};

static int sensors_acc_power_init(struct i2c_client *client)
{
	int rc;
	...
	rc = regulator_bulk_get(dev, ARRAY_SIZE(regs_8921_l9_sensor), regs_8921_l9_sensor);
	if (rc) {
		dev_err(dev, "%s: %d: could not get regulators 8921_l9_sensor: %d\n",__func__,__LINE__,rc);
		goto out;
	}

    rc = regulator_bulk_set_voltage(ARRAY_SIZE(regs_8921_l9_sensor), regs_8921_l9_sensor);
	if (rc) {
		dev_err(dev, "%s: %d: could not set_voltage 8921_l9_sensor: %d\n",__func__,__LINE__,rc);
		goto out;
	}
    
	regulator_bulk_enable(ARRAY_SIZE(regs_8921_l9_sensor), regs_8921_l9_sensor);
	msleep(200);

    /*
    regulator_bulk_disable(ARRAY_SIZE(8921_l9_sensor),8921_l9_sensor);
    regulator_bulk_free(ARRAY_SIZE(8921_l9_sensor),8921_l9_sensor);
    */
...
int sensor_acc_lsm330d_gpio_int_init(unsigned pin_num, char *pin_name)
{
    ...
    rc = gpio_request(pin_num, pin_name);
	if (rc)    
    {
        pr_err("%s: gpio(%d) is busy and then to free it\n", __func__, pin_num);
        
        gpio_free(pin_num);
        rc = gpio_request(pin_num, "acc_lsm330d_gpio_init");
        if (rc) 
        {
    ...
    rc = gpio_tlmm_config(GPIO_CFG(pin_num, 0,
...
free_acc_lsm330d_gpio:
	gpio_free(pin_num);
	gpio_tlmm_config(GPIO_CFG(pin_num, 0,
				GPIO_CFG_INPUT, GPIO_CFG_NO_PULL,
				GPIO_CFG_2MA), GPIO_CFG_DISABLE);
	return rc;
}

int sensor_acc_lsm330d_gpio_int_exit(unsigned pin_num)
{
	gpio_free(pin_num);
	gpio_tlmm_config(GPIO_CFG(pin_num, 0,
				     GPIO_CFG_INPUT, GPIO_CFG_NO_PULL,
...
int sensor_acc_lsm330d_gpio_init(void)
{
    int rc;
	printk(KERN_INFO "----------%s: %d: enter \n", __func__,__LINE__);
    rc = sensor_acc_lsm330d_gpio_int_init(GPIO_G_SENSOR_INT1, "acc_lsm330d_gpio_init1");
    if (rc)
    {
        return rc;
    }

    rc = sensor_acc_lsm330d_gpio_int_init(GPIO_G_SENSOR_INT2, "acc_lsm330d_gpio_init2");
    if (rc)
    {
        return rc;
    }
...
int sensor_acc_lsm330d_gpio_exit(void)
{
	printk(KERN_INFO "----------%s: %d: enter \n", __func__,__LINE__);
    sensor_acc_lsm330d_gpio_int_exit(GPIO_G_SENSOR_INT1);
    sensor_acc_lsm330d_gpio_int_exit(GPIO_G_SENSOR_INT2);    
	printk(KERN_INFO "----------%s: %d: exit \n", __func__,__LINE__);
    return 0;
}
...
struct lsm330dlc_acc_platform_data sensors_acc_lsm330d_pdata = {
...
#if defined (CONFIG_ZTEMT_SENSORS_NUBIA_Z5_MINI)
	.axis_map_x = 1,
	.axis_map_y = 0,
	.axis_map_z = 2,
	...
	.negate_z = 1,
#endif	
#if defined (CONFIG_ZTEMT_SENSORS_NUBIA_Z5S_MINI)
	.axis_map_x = 1,
	.axis_map_y = 0,
	.axis_map_z = 2,
...
int sensor_gyro_lsm330d_gpio_int_exit(unsigned pin_num)
{
	gpio_free(pin_num);
	gpio_tlmm_config(GPIO_CFG(pin_num, 0,
				     GPIO_CFG_INPUT, GPIO_CFG_NO_PULL,
...
int sensor_gyro_lsm330d_gpio_init(void)
{
    int rc;
    
	printk(KERN_INFO "----------%s: %d: enter \n", __func__,__LINE__);
    rc = sensor_gyro_lsm330d_gpio_int_init(GPIO_GYRO_INT, "gyro_lsm330d_gpio_init");
    if (rc)
    {
        return rc;
    }
...
int sensor_gyro_lsm330d_gpio_exit(void)
{
	printk(KERN_INFO "----------%s: %d: enter \n", __func__,__LINE__);
    sensor_gyro_lsm330d_gpio_int_exit(GPIO_GYRO_INT);
	printk(KERN_INFO "----------%s: %d: exit \n", __func__,__LINE__);
    return 0;
}
...
int sensor_press_lps331ap_gpio_int_init(unsigned pin_num, char *pin_name)
{
    int rc;
...
int sensor_press_lps331ap_gpio_int_exit(unsigned pin_num)
{
	gpio_free(pin_num);
	gpio_tlmm_config(GPIO_CFG(pin_num, 0,
				     GPIO_CFG_INPUT, GPIO_CFG_NO_PULL,
...
int sensor_press_lps331ap_gpio_init(void)
{
    int rc;
    
	printk(KERN_INFO "----------%s: %d: enter \n", __func__,__LINE__);
    
    rc = sensor_gyro_lsm330d_gpio_int_init(GPIO_PRESS_INT1, "press_lps331ap_gpio_init1");
    if (rc)
    {
        return rc;
    }

    rc = sensor_gyro_lsm330d_gpio_int_init(GPIO_PRESS_INT2, "press_lps331ap_gpio_init2");
    if (rc)
    {
        return rc;
    }
...
int sensor_press_lps331ap_gpio_exit(void)
{
	printk(KERN_INFO "----------%s: %d: enter \n", __func__,__LINE__);
    sensor_gyro_lsm330d_gpio_int_exit(GPIO_PRESS_INT1);
    sensor_gyro_lsm330d_gpio_int_exit(GPIO_PRESS_INT2);
	printk(KERN_INFO "----------%s: %d: exit \n", __func__,__LINE__);
    return 0;
}


struct lps331ap_prs_platform_data sensors_press_lps331ap_pdata = {
    .gpio_init = sensor_press_lps331ap_gpio_init,
    .gpio_exit = sensor_press_lps331ap_gpio_exit,
...
#ifdef CONFIG_ZTEMT_SENSORS_COMPASS_AKM8963
#define GPIO_COMPASS_INT    56

#if defined (CONFIG_ZTEMT_SENSORS_NUBIA_Z5_MINI) || defined (CONFIG_ZTEMT_SENSORS_NUBIA_Z5)
#define COMPASS_LAYOUTS     1
#endif

#if defined (CONFIG_ZTEMT_SENSORS_NUBIA_Z5S_MINI)
#define COMPASS_LAYOUTS     4
#endif
int sensor_akm8963_gpio_init(void)
{
    int rc;

	printk(KERN_INFO "----------%s: %d: enter \n", __func__,__LINE__);

    rc = gpio_request(GPIO_COMPASS_INT, "akm8963_gpio_int");
	if (rc)    
    {
        pr_err("%s: gpio(%d) is busy and then to free it\n", __func__, GPIO_COMPASS_INT);
        gpio_free(GPIO_COMPASS_INT);
    ...
    rc = gpio_tlmm_config(GPIO_CFG(GPIO_COMPASS_INT, 0,
                          GPIO_CFG_INPUT, GPIO_CFG_NO_PULL,
                          GPIO_CFG_2MA), GPIO_CFG_ENABLE);
    if (rc) {
       pr_err("%s: gpio_tlmm_config for %d failed\n",
    ...
    rc = gpio_direction_input(GPIO_COMPASS_INT);
    if (rc < 0) {
        pr_err("%s: unable to set the direction of gpio %d\n",
            __func__, GPIO_COMPASS_INT);
        goto free_akm8963_gpio;
    }
...
free_akm8963_gpio:
	gpio_free(GPIO_COMPASS_INT);
	gpio_tlmm_config(GPIO_CFG(GPIO_COMPASS_INT, 0,
				GPIO_CFG_INPUT, GPIO_CFG_NO_PULL,
				GPIO_CFG_2MA), GPIO_CFG_DISABLE);
	return rc;
}

 int sensor_akm8963_gpio_exit(void)
{
	gpio_free(GPIO_COMPASS_INT);
	gpio_tlmm_config(GPIO_CFG(GPIO_COMPASS_INT, 0,
				     GPIO_CFG_INPUT, GPIO_CFG_NO_PULL,
...
static struct akm8963_platform_data sensors_compass_akm8963_pdata= {
	.layout  = COMPASS_LAYOUTS,
	.outbit = 1,
	.gpio_DRDY         = GPIO_COMPASS_INT,
	.gpio_RST = 0,
	.gpio_init     = sensor_akm8963_gpio_init,
	.gpio_exit    = sensor_akm8963_gpio_exit,
};
#endif //CONFIG_SENSORS_COMPASS_AKM8963

#ifdef CONFIG_ZTEMT_SENSORS
static struct i2c_board_info __initdata pdata_sensors_boardinfo[] = {

    #ifdef CONFIG_ZTEMT_VIRTUAL_SENSOR_DEV
    {
        I2C_BOARD_INFO(ZTEMT_VIRTUAL_SENSOR_DEV_NAME, 0x11),
        .platform_data = &ztemt_virtual_sensor_dev_pdata,
    },
    #endif

    #ifdef CONFIG_ZTEMT_SENSORS_G_SENSOR_LSM330D
    {
		I2C_BOARD_INFO(LSM330DLC_ACC_DEV_NAME, 0x18),
		.platform_data = &sensors_acc_lsm330d_pdata,
	},
	#endif
    ...
    #ifdef CONFIG_ZTEMT_SENSORS_COMPASS_AKM8963
    {
		...
		.platform_data = &sensors_compass_akm8963_pdata,
	},
	#endif
    
	#ifdef CONFIG_ZTEMT_SENSORS_GYRO_LSM330D 
	{
		I2C_BOARD_INFO(LSM330DLC_GYR_DEV_NAME,0x6A),
		.platform_data = &sensors_gyro_lsm330d_pdata,
	},
    #endif

    #ifdef CONFIG_ZTEMT_SENSORS_PRESS_LPS331AP
    {
            I2C_BOARD_INFO(LPS331AP_PRS_DEV_NAME,0x5C),
            .platform_data = &sensors_press_lps331ap_pdata,
    },
    #endif
};
#endif
//===================================== sensor platform data end=============================================//
...
#define SII8334_plus 0x0  //Define sii8334's I2c Address of all pages by the status of CI2CA
#define SII8334_reset 31
#define SII8334_int 43
...
{	
	...
	static int ret;
	if (is_firsttime) {
		ret = gpio_request(SII8334_reset,
			"SII8334_reset");
		if (ret < 0) {
			pr_err("GPIO SII8334_reset request failed: %d\n", ret);
		}
		is_firsttime = false;
	}
	gpio_direction_output(SII8334_reset, 0);
	msleep(5);
	gpio_set_value(SII8334_reset, 3);
}
static void Sii8334_gpio_input(void)
{
	static bool is_firsttime = true;
	static int ret;
	if (is_firsttime) {
		ret = gpio_request(SII8334_int,
			"SII8334_int");
		if (ret < 0) {
			pr_err("GPIO SII8334_int request failed: %d\n", ret);
		}
		is_firsttime = false;
	}
	//io_direction_input(SII8334_int);
	gpio_tlmm_config(GPIO_CFG(43, 0,
		GPIO_CFG_INPUT, GPIO_CFG_PULL_UP,
		GPIO_CFG_2MA), GPIO_CFG_DISABLE);
}
static int Sii8334_power(int on)
{
	...
	if (is_firsttime) {
		reg_l23 = regulator_get(NULL,
						"mhl_panel_1p8");
		if (IS_ERR(reg_l23)) {
				pr_err("could not get mhl_panel_1p8, rc = %ld\n",
						PTR_ERR(reg_l23));
				return -ENODEV;
		}
		rc = regulator_set_voltage(reg_l23, 1800000, 1800000);
		if (rc) {
				pr_err("set_voltage reg_l23 failed, rc=%d\n", rc);
				return -EINVAL;
		}	
		reg_l12 = regulator_get(NULL,
						"mhl_panel_1p2");
		if (IS_ERR(reg_l12)) {
				pr_err("could not get mhl_panel_1p2, rc = %ld\n",
						PTR_ERR(reg_l12));
				return -ENODEV;
		}
		rc = regulator_set_voltage(reg_l12, 1200000, 1200000);
		if (rc) {
				pr_err("set_voltage reg_l12 failed, rc=%d\n", rc);
				return -EINVAL;
		}
		is_firsttime = false;
	}
	...
	if (on) {
		...
		if (rc) {
			pr_err("enable reg_l23 failed, rc=%d\n", rc);
			return -ENODEV;
		}
		rc = regulator_enable(reg_l12);
		if (rc) {
			pr_err("enable reg_l12 failed, rc=%d\n", rc);
			return -ENODEV;
		}
		msleep(2);
	} else {
...
static struct msm_mhl_platform_data mhl_platform_data = {
	.reset = Sii8334_reset,
	.gpio_input = Sii8334_gpio_input,
	.mhl_power = Sii8334_power,
};

#if 0
static struct msm_mhl_platform_data mhl_platform_data = {
	.irq = MSM_GPIO_TO_INT(SII8334_int),
	.gpio_mhl_int = SII8334_int,
	.gpio_mhl_reset = SII8334_reset,
	.gpio_mhl_power = 0,
	.gpio_hdmi_mhl_mux = 0,
};
#endif
...
#ifdef CONFIG_ZTEMT_BREATH_LED
static struct i2c_board_info aw9106b_device_info[] __initdata = {
	{
		I2C_BOARD_INFO("aw9106b", 0x5b),
	},
};
#endif
...
/*added by congshan 20121008 end*/
	...
	/* ZTEMT Added by LiuYongfeng, 2013/2/25 add uart devices*/
...
static struct platform_device *early_common_devices[] __initdata = {
	...
	&apq8064_device_dmov,
#if defined(CONFIG_SERIAL_MSM_HSL) || defined(CONFIG_ZTEMT_MHL_8064)
	/* nothing,add by liuyongfeng*/
#else
...
static struct platform_device *cdp_devices[] __initdata = {
	//&apq8064_device_uart_gsbi1,//yfliu
	//&apq8064_device_uart_gsbi7,//yfliu
...
static struct msm_i2c_platform_data mpq8064_i2c_qup_gsbi5_pdata = {
	.clk_freq = 100000,
	.src_clk_rate = 24000000,
/* ZTEMT Added by LiuYongfeng, 2012/10/10 uart and i2c share gsbi5*/
#ifdef CONFIG_SERIAL_MSM_HSL
	.msm_i2c_config_gpio = gsbi_qup_i2c_gpio_config,//Dump function
...
#define UART_I2C_PROTOCOL (0x06 << 4)	//Protocol for UART/I2C
static void __init apq8064_i2c_init(void)
{
	void __iomem *gsbi_mem;
	/* ZTEMT Added by LiuYongfeng, 2012/10/16 */
	void __iomem *gsbi_ctrl = ioremap_nocache(MSM_GSBI5_PHYS,4);
...
/* ZTEMT Added by LiuYongfeng, 2012/10/11 */
#define GPIO_KEY_VOLUME_UP		PM8921_GPIO_PM_TO_SYS(38) //PM8921_GPIO_PM_TO_SYS(35)
#define GPIO_KEY_VOLUME_DOWN_PM8921	PM8921_GPIO_PM_TO_SYS(35) //PM8921_GPIO_PM_TO_SYS(38)
/* ZTEMT END */
#define GPIO_KEY_VOLUME_DOWN_PM8917	PM8921_GPIO_PM_TO_SYS(30)
#ifdef CONFIG_ZTEMT_CAMERA_KEY
#define GPIO_KEY_CAM_FOCUS		PM8921_GPIO_PM_TO_SYS(37)
#define GPIO_KEY_CAM_SNAP		PM8921_GPIO_PM_TO_SYS(36)
#else
...
#endif
	/*added by congshan 20121008 end*/
//===================================== sensor devices register begain   ==========================================//
#ifdef CONFIG_ZTEMT_SENSORS
    {
        I2C_SURF | I2C_LIQUID | I2C_FFA,
        APQ_8064_GSBI2_QUP_I2C_BUS_ID,    
        pdata_sensors_boardinfo,
        ARRAY_SIZE(pdata_sensors_boardinfo),
    },
#endif
//=================================== sensor devices register begain   ============================================//
#ifdef CONFIG_ZTEMT_BREATH_LED
	{
		...
		APQ_8064_GSBI5_QUP_I2C_BUS_ID,
		aw9106b_device_info,
		ARRAY_SIZE(aw9106b_device_info),
	},
#endif
};
...
#ifdef CONFIG_ZTEMT_HW_BOARD_TYPE

enum nubia_board_type {
	Z5SMINI_WITH_NFC_HDMI = 0,
	Z5SMINI_WITHOUT_NFC_HDMI,
	UNKNOWN_BOARD_TYPE,
};

struct board_type_map {
	unsigned int board_type;
	char board_type_name[50];
};

static const struct board_type_map nubia_board_type_map[] = {
	{Z5SMINI_WITH_NFC_HDMI,"Z5SMINI_WITH_NFC_HDMI"},
	{Z5SMINI_WITHOUT_NFC_HDMI,"Z5SMINI_WITHOUT_NFC_HDMI"}
};

static enum nubia_board_type hw_board_type = UNKNOWN_BOARD_TYPE; 

static int  ztemt_board_type_setup(char *param)
{
	int magic_num = 0;
    get_option(&param, &magic_num);
	printk("_ztemt_board_type_setup: magic_num=0x%x.\n", magic_num);
	hw_board_type = magic_num;
    return 0;
}
early_param("board_type", ztemt_board_type_setup);

int zte_get_board_type(void)                                        
{                                                                                                                           
	return nubia_board_type_map[hw_board_type].board_type;                                                          
}                                                                          
EXPORT_SYMBOL(zte_get_board_type);

static struct kobject *board_type_kobject = NULL;
static ssize_t board_type_show(struct device *dev, 
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, 32, "%s\n", nubia_board_type_map[hw_board_type].board_type_name);
}

static DEVICE_ATTR(board_type, 0444, board_type_show, NULL);
static int board_type_init(void)
{
	int ret;

	board_type_kobject = kobject_create_and_add("ztemt_board_type", NULL);
	if(board_type_kobject == NULL) {
		ret = -ENOMEM;
		goto err1;
	}

	ret = sysfs_create_file(board_type_kobject, &dev_attr_board_type.attr);
	if(ret){
		goto err;
	}
...
err:
	kobject_del(board_type_kobject);
err1:
	printk("board_type_init: Failed to create sys file\n");
	return ret;
}
#endif
...
#if defined( CONFIG_ZTEMT_LCD_Z5MINI2)
#if defined( CONFIG_ZTEMT_LCD_Z5MINI2_SIZE_4P7)
static enum nubia_lcd_type mini2_lcd_ic_type = 0; 

static int  mini2_lcd_type_setup(char *param)
{
	unsigned int lcd_ic_id = 0;
    get_option(&param, &lcd_ic_id);
	printk("mini2_lcd_type_setup: lcd_ic_id=0x%x.\n", lcd_ic_id);
#ifndef CONFIG_ZTEMT_LCD_CMD_MODE
	if (lcd_ic_id <= 0) {
		// r69431 or no lcd
		mini2_lcd_ic_type = SHARP_R69431;
	} else {
		// otm1282a
		mini2_lcd_ic_type = TIANMA_OTM1282A;
	}
#else
	if (0x9431 == lcd_ic_id) {
		// r69431
		mini2_lcd_ic_type = SHARP_R69431;
	} else if (0x40 == lcd_ic_id) {
		// otm1282a
		mini2_lcd_ic_type = TIANMA_OTM1282A;
	} else {
		// no lcd
		mini2_lcd_ic_type = NO_LCD;
	}
#endif
	
    return 0;
}
early_param("lcdc_type", mini2_lcd_type_setup);

enum nubia_lcd_type mini2_get_lcd_type(void)                                        
{                                                                                                                           
	return mini2_lcd_ic_type;                                                          
}                                                                          
EXPORT_SYMBOL(mini2_get_lcd_type);
#endif
#endif
...
#ifdef CONFIG_TOUCHSCREEN_CYTTSP_I2C //yfliu remove useless code
	if (machine_is_apq8064_mtp() &&
...
MACHINE_START(APQ8064_MTP, "nubia")
	.map_io = apq8064_map_io,
	.reserve = apq8064_reserve,
	.init_irq = apq8064_init_irq,
	.handle_irq = gic_handle_irq,
