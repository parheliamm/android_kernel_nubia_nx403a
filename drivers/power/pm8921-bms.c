...
#ifdef  ZTEMT_BMS_DEBUG
static int bmslog_level = 0;
module_param(bmslog_level, int, 0644);

#define BMSLOG_INFO(fmt, args...) \
		if (bmslog_level) \
				...
				int batt_temp_decidegc)
{
	...
	if (chip->prev_last_good_ocv_raw == OCV_RAW_UNINITIALIZED) {
		...
		BMSLOG_INFO("-socraw-1--last_good_ocv_uv=%d\n",raw->last_good_ocv_uv);
		...
		if (is_warm_restart(chip)
			...
			BMSLOG_INFO("-socraw-11--last_good_ocv_uv=%d\n",raw->last_good_ocv_uv);
		}
		...
		chip->last_cc_uah = 0;
		BMSLOG_INFO("-socraw-2--last_good_ocv_uv=%d\n",raw->last_good_ocv_uv);
	} else {
					...
					int batt_temp, int chargecycles)
{
	...
	BMSLOG_INFO("--RUC=RC-CCmAh-UUC:%d=%d-%d-%d  uAh\n", 
		remaining_usable_charge_uah,remaining_charge_uah,cc_uah,unusable_charge_uah);
	...
	BMSLOG_INFO("--soc=100*RUC/(FCC-UUC):%d=100*%d/(%d-%d)\n", 
				soc,remaining_usable_charge_uah,fcc_uah,unusable_charge_uah);
...
static int recalculate_soc(struct pm8921_bms_chip *chip)
{
	...
	BMSLOG_INFO("__CalcSoc_END_______\n");
...
void ztemt_backup_soc(int soc)
{
	...
	get_batt_temp(the_chip, &batt_temp);
    backup_soc_and_iavg(the_chip, batt_temp, soc);
}
#endif
...
static int set_battery_data(struct pm8921_bms_chip *chip)
{
	...
	chip->fcc = ztemt_2000mAh_data.fcc;
	chip->fcc_temp_lut = ztemt_2000mAh_data.fcc_temp_lut;
	chip->fcc_sf_lut = ztemt_2000mAh_data.fcc_sf_lut;
	chip->pc_temp_ocv_lut = ztemt_2000mAh_data.pc_temp_ocv_lut;
	chip->pc_sf_lut = ztemt_2000mAh_data.pc_sf_lut;
	chip->rbatt_sf_lut = ztemt_2000mAh_data.rbatt_sf_lut;
	chip->default_rbatt_mohm = ztemt_2000mAh_data.default_rbatt_mohm;
	chip->delta_rbatt_mohm = ztemt_2000mAh_data.delta_rbatt_mohm;	
	return 0;
    #elif defined(CONFIG_ZTEMT_BATTERY_2300MAH)
	chip->fcc = ztemt_2300mAh_data.fcc;
	chip->fcc_temp_lut = ztemt_2300mAh_data.fcc_temp_lut;
	chip->fcc_sf_lut = ztemt_2300mAh_data.fcc_sf_lut;
	chip->pc_temp_ocv_lut = ztemt_2300mAh_data.pc_temp_ocv_lut;
	chip->pc_sf_lut = ztemt_2300mAh_data.pc_sf_lut;
	chip->rbatt_sf_lut = ztemt_2300mAh_data.rbatt_sf_lut;
	chip->default_rbatt_mohm = ztemt_2300mAh_data.default_rbatt_mohm;
	chip->delta_rbatt_mohm = ztemt_2300mAh_data.delta_rbatt_mohm;
	return 0;		
    #endif
