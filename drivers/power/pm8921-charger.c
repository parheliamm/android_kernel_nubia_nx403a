...
#define PMIC_HIGH_TEMP 85000
#define CHG_MONITOR_PERIOD_MS	10000
#define ZEMT_POWER_DEBUG 0
#define CHGBAT_TEMP_DIS_BIT	BIT(2)

//0.1 degree   eg:470 = 47C
#define CHG_TEMP_HIGH1   500
#define CHG_TEMP_HIGH2   470
#define CHG_TEMP_LOW1     -60
#define CHG_TEMP_LOW2     -50

#define BATT_TEMP_HIGH   480
...
struct delayed_work		batt_monitor_work;
...
extern void ztemt_backup_soc(int soc);
extern void *restart_reason;
static int is_chg_enable;
...
static int is_charger_online(void)
{
    return monitor_st.charger_online;
}
static void set_charger_status(int present)
{
    monitor_st.charger_online = present;
}
static int is_chg_batt_temp_abnormal(void)
{
    return monitor_st.temp_abnormal;
}
static void set_chg_batt_temp_st(enum batt_temp_st temp_status)
{
    monitor_st.temp_abnormal = temp_status;
}
static void set_batt_temp(int temp)
{
...
static void  check_start_monitor_work(int chg_in)
{
	set_charger_status(chg_in);

	if(chg_in){
		pm_chg_auto_enable(the_chip, 1);
        schedule_delayed_work(&batt_monitor_work,
				  round_jiffies_relative(msecs_to_jiffies
							(CHG_MONITOR_PERIOD_MS)));
	}
}
#endif/*CONFIG_ZTEMT_CHARGE*/

#ifdef CONFIG_ZTEMT_HARDWARE_ID

static const struct hardware_id_map_st hardware_id_map[] = {
	{0, 80,V5_HW_A,"QB8064V1A_MB_A"},  //id_mv=0
	{100, 250,V5_HW_C1,"QB8064V1A_MB_C1"},  //id_mv=162
	{300, 520,V5_HW_C2,"QB8064V1A_MB_C2"},  //id_mv=413
	{580, 760,V5_HW_D,"QB8064V1A_MB_D"},   //id_mv=689
	{800, 1000,V5_HW_E,"QB8064V1A_MB_E"}   //id_mv=900
};
hw_version_type ztemt_hw_id = V5_HW_UN;
...
int ztemt_get_hw_id(void)
{
    ...
    rc = pm8xxx_adc_read(ADC_MPP_1_AMUX4,&adc_result);
    if (rc) {
		pr_err("error reading channel ADC_MPP_1_AMUX4 = %d, rc = %d\n",
					ADC_MPP_1_AMUX4, rc);
		return rc;
	}
    ...
    ztemt_hw_id = ztemt_get_hardware_type(
						...
						hw_id_uv);
	PMLOG_DEBUG("hw_id_uv=%lld uv hw_id=%d hw_ver=%s\n",
		hw_id_uv,ztemt_hw_id,hardware_id_map[ztemt_hw_id].hw_ver);
	get_hw = 1;
	return ztemt_hw_id;
}
...
#endif/*CONFIG_ZTEMT_HARDWARE_ID*/
...
static int get_prop_batt_capacity(struct pm8921_chg_chip *chip)
{
    ...
    #ifdef CONFIG_ZTEMT_CHARGE
	//when charging full and soc=100,after long time then soc<=99 and charger is still online,start charging again
    if(percent_soc == 100 && is_charger_online())
		chg_full = 1;
	else if(percent_soc < 95 || !is_charger_online())
		chg_full = 0;

	if (chg_full && percent_soc <= 99 && is_charger_online() && !is_chg_batt_temp_abnormal()){
		chg_fsm = pm_chg_get_fsm_state(chip);
		if(chg_fsm != FSM_STATE_FAST_CHG_7 && !is_chg_enable){
			pm_chg_auto_enable(chip, !charging_disabled);
			printk("---start charging again\n");
		}
	}
	
    /* As charging full and soc=100,after long time then soc=99,98,97
           set the soc=100 when  batt_current<5mA and charger is still online   */
	if( chip->recent_reported_soc==100 && (percent_soc==99 || percent_soc==98 || percent_soc==97) ){	
		get_prop_batt_current(chip,&batt_currt);
        //if it is charging,set to 100%
		if( batt_currt < 5000 && is_charger_online() ){
			percent_soc = 100;
			if(pm_chg_get_fsm_state(chip) != FSM_STATE_FAST_CHG_7)
				pm_chg_auto_enable(chip, !charging_disabled);
			printk("---set percent_soc to 100\n");
		}
	}
	if(percent_soc == 100)
		ztemt_backup_soc(percent_soc);
    #endif
fail_voltage:
...
int get_batt_therm_mv(void)
{
    return batt_therm_mv;
}

void store_batt_therm_mv(int batt_temp_mv)
{
...
static int get_prop_batt_temp(struct pm8921_chg_chip *chip, int *temp)
{
	...
	struct pm8xxx_adc_chan_result result;
	#ifdef CONFIG_ZTEMT_CHARGE
	struct pm8xxx_adc_chan_result pmic_result;
	static int last_batt_temp = 300;
	static int hot_flag = 0;
	#endif
	...
	if ( result.physical > MAX_TOLERABLE_BATT_TEMP_DDC ){
		if(hot_flag == 0){
        	//enable the battery temperature monitor 
			rc = pm_chg_masked_write(chip, CHG_CNTRL_2,CHGBAT_TEMP_DIS_BIT, 0);
			*temp = last_batt_temp;
			hot_flag = 1;
			PMLOG_DEBUG("First hot batt_temp=%lld\n",result.physical);
		}else{
       		rc = pm8xxx_adc_read(CHANNEL_DIE_TEMP,&pmic_result);
			if(rc){
				pr_err("error reading adc channel = %d, rc = %d\n",
							CHANNEL_DIE_TEMP, rc);
        	}
			PMLOG_DEBUG("batt_temp=%lld pmic_temp=%lld batt_therm_mv=%d\n",result.physical,pmic_result.physical,get_batt_therm_mv());

			if( pmic_result.physical < PMIC_HIGH_TEMP || get_batt_therm_mv()<BATT_THERM_MIN_MV ){
				*temp = last_batt_temp;
				hot_flag = 0;
			}else{
				*temp = (int)result.physical;
		     	pr_err("BATT_TEMP= %d > 68degC,pmic_temp=%lld device will be shutdown\n",
							(int) result.physical,pmic_result.physical);
			}
		}
	}else{
		*temp = (int)result.physical;
...
static irqreturn_t usbin_valid_irq_handler(int irq, void *data)
{
	#ifdef CONFIG_ZTEMT_CHARGE
	int chg_in;
	chg_in = is_usb_chg_plugged_in(the_chip);
	PMLOG_DEBUG("usb_online=%d\n",chg_in);
	set_chg_batt_temp_st(BATT_TEMP_GOOD);
	check_start_monitor_work(chg_in);
	#endif
	if (usb_target_ma)
...
/**************************************************************************
ADDED by swang 2012.12.06
Because the hw monitoring the battery temperature is not accurate,
so use the delayed_work to monitor the battery temperature and control the charging.
**************************************************************************/
#ifdef CONFIG_ZTEMT_CHARGE
static int eoc_end = 0;
static void batt_monitor_worker(struct work_struct *work)
{
    int batt_temp;
    int batt_currt;
	
	get_prop_batt_temp(the_chip,&batt_temp);
	get_prop_batt_current(the_chip,&batt_currt);
	
	PMLOG_DEBUG("end=%d SOC=%d mV=%d mA=%d T=%d fsm=%d\n",
			eoc_end,
			get_prop_batt_capacity(the_chip),
			get_prop_battery_uvolts(the_chip)/1000,
			batt_currt/1000,
			batt_temp,
	...
	if( batt_temp > CHG_TEMP_LOW2 && batt_temp < CHG_TEMP_HIGH2 && is_chg_batt_temp_abnormal() ){
		PMLOG_INFO("====battery temp is nomal T=%d start charging!\n",batt_temp);
		pm_chg_auto_enable(the_chip, 1);
		set_chg_batt_temp_st(BATT_TEMP_GOOD);
	}else if( (batt_temp > CHG_TEMP_HIGH1 || batt_temp < CHG_TEMP_LOW1) && !is_chg_batt_temp_abnormal() ){
		PMLOG_INFO("====battery temp is abnormal T=%d stop charging!\n",batt_temp);
...
static void eoc_worker(struct work_struct *work)
{
	...
	if (count == CONSECUTIVE_COUNT) {
		...
		check_temp_thresholds(chip);
		#ifdef CONFIG_ZTEMT_CHARGE
		if( is_chg_batt_temp_abnormal() == BATT_TEMP_GOOD && (end != CHG_NOT_IN_PROGRESS))
...
static void __devinit determine_initial_state(struct pm8921_chg_chip *chip)
{
	...
	if (chip->usb_present || chip->dc_present) {
		schedule_delayed_work(&chip->unplug_check_work,
			msecs_to_jiffies(UNPLUG_CHECK_WAIT_PERIOD_MS));
	//ZTEMT ADDED by swang 2012.12.6 begin to monitor the battery temperature in system starting
	#ifdef CONFIG_ZTEMT_CHARGE
		check_start_monitor_work(chip->usb_present);
	#endif
		pm8921_chg_enable_irq(chip, CHG_GONE_IRQ);
...
#define BOOTLOADER_RESTART_REASON   0x77665500
#define RECOVERY_RESTART_REASON   0x77665502
#define OEM_RESTART_REASON   0x6f656d00
#define COMMON_RESTART_REASON   0x77665501
#define BATT_MIN_UV  2800000
#define BATT_MAX_UV  4450000

extern  u8 pm_poweron_reason;
u8 goto_poweroffchg = 0;
static struct wake_lock poweoffchg_wake_lock;

int get_poweroffchg_status(void)
{
	return goto_poweroffchg;
}
EXPORT_SYMBOL_GPL(get_poweroffchg_status);

static ssize_t poweroffchg_info_show(struct device *dev, 
		struct device_attribute *attr, char *buf)
{
    if(buf)
    	*buf = goto_poweroffchg;
	return 1;
}

static ssize_t poweroffchg_info_store(struct device *dev, 
		struct device_attribute *attr, const char *buf, size_t count)
{
     return count;
}

static DEVICE_ATTR(info, 0444, poweroffchg_info_show, poweroffchg_info_store);

/* ZTEMT END */
static struct kobject *poweroffchg_kobject = NULL;
static int   poweroffchg_init(void)
{
	int ret;

	poweroffchg_kobject = kobject_create_and_add("ztemt_poweroff_charge", NULL);
	if(poweroffchg_kobject == NULL) {
		ret = -ENOMEM;
		goto err1;
	}

	ret = sysfs_create_file(poweroffchg_kobject, &dev_attr_info.attr);
	if(ret){
		goto err;
	}
...
err:
	kobject_del(poweroffchg_kobject);
err1:
	printk("Poweroffchg: Failed to create sys file\n");
	return ret;
}
...
void poweroff_charge_check(void)
{
	...
	is_dc_on = is_dc_chg_plugged_in(the_chip);
	is_batt_on = pm_chg_get_rt_status(the_chip, BATT_INSERTED_IRQ);
	...
	if( pm_poweron_reason == PM8XXX_RESTART_KPD )
		goto_poweroffchg = 0;
	else if( app_reboot_reason == BOOTLOADER_RESTART_REASON ||
			 app_reboot_reason == RECOVERY_RESTART_REASON ||
			 (app_reboot_reason & 0x00) == OEM_RESTART_REASON ||
			 app_reboot_reason == COMMON_RESTART_REASON )
		goto_poweroffchg = 0;
    else if( (is_usb_on||is_dc_on||pm_poweron_reason==PM8XXX_RESTART_CHG) && is_batt_on )
	    goto_poweroffchg = 1;
	else
		goto_poweroffchg = 0;
...
static int pm8921_charger_resume(struct device *dev)
{
	...
	#ifdef CONFIG_ZTEMT_POWEROFF_CHARGE
	if( goto_poweroffchg ){
		wake_lock_timeout(&poweoffchg_wake_lock, 10 * HZ);
	}
	#endif
...
#define APQ_GPIO_32   32
#define APQ_GPIO_82   82
static void board_type_gpio_config(void)
{
	int ret;
			
	ret = gpio_request(APQ_GPIO_32, "hw-id0");
	if (ret) {
		pr_err("%s: fail gpio_request(%d)=%d\n", __func__,APQ_GPIO_32, ret);
	}
	gpio_tlmm_config(GPIO_CFG(APQ_GPIO_32, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA),GPIO_CFG_ENABLE);
	
	ret = gpio_request(APQ_GPIO_82, "hw-id1");
	if (ret) {
		pr_err("%s: fail gpio_request(%d)=%d\n", __func__,APQ_GPIO_82, ret);
	}
	gpio_tlmm_config(GPIO_CFG(APQ_GPIO_82, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA),GPIO_CFG_ENABLE);
}
#endif
...
static int __devinit pm8921_charger_probe(struct platform_device *pdev)
{
	...
	#ifdef CONFIG_ZTEMT_CHARGE
	INIT_DELAYED_WORK(&batt_monitor_work, batt_monitor_worker);
	#endif
	...
	if (chip->update_time)
		schedule_delayed_work(&chip->update_heartbeat_work,
				      round_jiffies_relative(msecs_to_jiffies
							(chip->update_time)));
	#ifdef CONFIG_ZTEMT_POWEROFF_CHARGE
	wake_lock_init(&poweoffchg_wake_lock, WAKE_LOCK_SUSPEND, "poweroffchg");
	poweroff_charge_check();
	#endif
	return 0;
